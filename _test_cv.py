...
work="_t2"; pout=os.path.join(work,"parser_out"); os.makedirs(pout,exist_ok=True)
m1=simulate_1min(n_minutes=60000, seed=11)
...
print("m15 pullback episodes:", len(pb), "| has resolve_time:", "resolve_time" in pb.columns)
co=labels["m15_continuation"]; print("m15 cont has resolve_time:", "resolve_time" in co.columns)

# new time-purged CV, real vs shuffled control
for tgt,clf in [("pullback_depth",False),("continuation_occurred",True)]:
    r = M.run_task(pb,tgt,classification=clf,name="real",n_splits=3)
    s = M.run_task(pb,tgt,classification=clf,name="shuf",n_splits=3,shuffle_y=True)
    def f(x): return None if x is None else {k:round(v,3) for k,v in x.metrics.items()}
    print(f"{tgt:24s} real={f(r)}  shuffled={f(s)}")
print("CV + shuffle path works.")
